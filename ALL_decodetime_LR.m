function [rate_matrices, posterior_means, posterior_trials, reshape_trials, bin_times, rm_bins_normal_means] = ALL_decodetime_LR(stage_numbers, wndw, bins, boxcar_shift) %, posterior_means, posterior_trials, reshape_trials
    ...
    for stage = stage_numbers
        ...
        for session = 1:length_sessions
            ...
            if ~isempty(clusters)
                ...
                %correct trials
                corrects = unique(eptrials(eptrials(:,9)==1 & eptrials(:,8)==2,6));
...
%merge rate_matrices
[rm_bin_means_train, rm_bins_test] = merge_rate_matrices_LR(rate_matrices);
...
size(rm_bin_means_train,3)
size(rm_bins_test,3)
    
    
IDs = repmat(1:size(rm_bin_means_train,2), 1, size(rm_bin_means_train,3));
training = reshape(rm_bin_means_train, [246 , size(rm_bins_test,2)*size(rm_bin_means_train,3)]);
[posterior_trials] = decode_time_LR(training, rm_bins_test, IDs, bins);

count = 0; 
count2 = 1; 
for i = 1:size(rm_bins_test,3)
