...
%       'train_ID' is a vector of pixel ids describing the rat was at each 
...
%
    ...
    %PRIOR PROBABILITY, P(A), i.e. the spatial occupancy distribution 
    %
    % P(A) = N(A) / sum(N(A))
    %
...
%
    ...
    %
        ...
        for tsamp = 1:nsamp
            ...
            P_B_A_cell(tsamp, :, :) = exp(o_spikes.*log(e_spikes)-e_spikes-gammaln(o_spikes+ones(size(o_spikes))));%equivalent and more stable
