function [eptrials, section_boundaries, lost_trials] = lv_trials(vid, ttun, spiketimes, REWARD_L, REWARD_R, ERROR_L, ERROR_R, ITI_ON, ITI_OFF, LEFT_ON, LEFT_OFF, RIGHT_ON, RIGHT_OFF, TRIAL_START, TRIAL_START_L, TRIAL_START_R, TRIAL_START_L_TURN, TRIAL_START_R_TURN)
...
%   ERROR_ L/R = first timestamp of licks on left/right error trials
%   ITI_ ON/OFF = first/last timestamp of intertrial interval
%   LEFT_ ON/OFF = first/last timestamp of left light being on
%   RIGHT_ ON/OFF = first/last timestamp of right light being on
%   TRIAL_START = first timestamp of every trial
%   TRIAL_START_ L/R = first timestamp of every left/right light trial
%   TRIAL_START_ L/R _TURN = first timestamp of every left/right turn trial
%
...
%   eptrials(:, 5) manual flag (rwd_L/R = 1/2, err_L/R = 3/4)
%   eptrials(:, 6) trial number
%   eptrials(:, 7) trial type (1 = L, 2 = R)
%   eptrials(:, 8) choice (1 = L, 2 = R)
%   eptrials(:, 9) trial accuracy (1 = C, 0 = E)
...
eptrials = flag_evts(eptrials, REWARD_L, REWARD_R, ERROR_L, ERROR_R, initial_clock);
    function eptrials = flag_evts(eptrials, REWARD_L, REWARD_R, ERROR_L, ERROR_R, initial_clock)
    ...
    %       Left error instant = 3
    %       Right reward instant = 4
    %
    %
    % INPUT VARIABLES
    %   eptrials = the overall output; a matrix of all relevant information
    ...
    %   ERROR_L = Video flag of left errors
    %   ERROR_R = Video flag of right errors
    %   initial_clock = the uncorrected session start time
    %
    % OUTPUT VARIABLE
    %   eptrials = the overall output; a matrix of all relevant information
    %
        ...
        EL_flags = [correct_time(ERROR_L, initial_clock)' nan(length(ERROR_L),3) repmat(3, size(ERROR_L'))];
        ER_flags = [correct_time(ERROR_R, initial_clock)' nan(length(ERROR_R),3) repmat(4, size(ERROR_R'))];

        %combine eptrials and flags
        eptrials = sortrows([eptrials; RL_flags; RR_flags; EL_flags; ER_flags]);
    end    
...
%add trial type (column 7)
eptrials = LR_trial_types(eptrials, trial_numbers, TRIAL_START, TRIAL_START_L, TRIAL_START_R);
    function eptrials = LR_trial_types(eptrials, trial_numbers, TRIAL_START, TRIAL_START_L, TRIAL_START_R)
    ...
    %   TRIAL_START = start times of each trial
    %   TRIAL_START_L = start times of each left trial
    %   TRIAL_START_R = start times of each right trial
    %
    % OUTPUT VARIABLE
    %   eptrials = the overall output; a matrix of all relevant information
    %
        ...
        left_trials = trial_numbers(ismember(TRIAL_START, TRIAL_START_L));
        right_trials = trial_numbers(ismember(TRIAL_START, TRIAL_START_R));

        %set rows
        eptrials(ismember(eptrials(:,6), left_trials), 7) = 1;
        eptrials(ismember(eptrials(:,6), right_trials), 7) = 2;
    end

%add choice type (column 8)
eptrials = LR_choice_types(eptrials, trial_numbers, TRIAL_START, TRIAL_START_L_TURN, TRIAL_START_R_TURN);
    function eptrials = LR_choice_types(eptrials, trial_numbers, TRIAL_START, TRIAL_START_L_TURN, TRIAL_START_R_TURN)
    % LR_choice_types adds a column of choice type to eptrials. This
    % indicates which direction the rat turned (not necessarily where the 
    % reward was located or which light was on).
    %
    %   Trial types are coded as follows...
    %       Rat turned left = 1
    %       Rat turned right = 2
    ...
    %
        ...
        %left turn and right turn trial numbers
        leftturn_trials = trial_numbers(ismember(TRIAL_START, TRIAL_START_L_TURN));
        rightturn_trials = trial_numbers(ismember(TRIAL_START, TRIAL_START_R_TURN));

        %set rows
        eptrials(ismember(eptrials(:,6), leftturn_trials), 8) = 1;
        eptrials(ismember(eptrials(:,6), rightturn_trials), 8) = 2;
...
eptrials = trial_accuracy(eptrials);
    ...
    %
        ...
        %find error rows
        errors = abs(eptrials(:,7) - eptrials(:,8));
        errors(isnan(errors)) = 0;
        error_idx = logical(errors);

        %set rows
        eptrials(error_idx, 9) = 0;
        eptrials(~error_idx, 9) = 1;
        eptrials(isnan(errors), 9) = NaN;
    end
...
eptrials = light_status(eptrials, initial_clock, LEFT_ON, LEFT_OFF, RIGHT_ON, RIGHT_OFF);
    function eptrials = light_status(eptrials, initial_clock, LEFT_ON, LEFT_OFF, RIGHT_ON, RIGHT_OFF)
    ...
    %   eptrials = the overall output; a matrix of all relevant information
    %   initial_clock = the uncorrected session start time
    %   LEFT_ON = Timestamps for left light turning on
    %   LEFT_OFF = Timestamps for left light turning off
    %   RIGHT_ON = Timestamps for right light turning on
    %   RIGHT_OFF = Timestamps for right light turning off
    %
    % OUTPUT VARIABLE
    %   eptrials = the overall output; a matrix of all relevant information
    % 
        ...
        %set rows for left light
        L_light_starts = (LEFT_ON - repmat(initial_clock, size(LEFT_ON)))./1000000; 
        L_light_ends = (LEFT_OFF - repmat(initial_clock, size(LEFT_OFF)))./1000000;
        L_light_starts_ends = [L_light_starts' L_light_ends'];    
        for L_light = 1:size(L_light_starts_ends,1)

            %find range
            [~,start_row] = min(abs(eptrials(:,1) - repmat(L_light_starts_ends(L_light, 1), size(eptrials(:,1)))));
            [~,end_row] = min(abs(eptrials(:,1) - repmat(L_light_starts_ends(L_light, 2), size(eptrials(:,1)))));
        ...
        %set rows for left light
        R_light_starts = (RIGHT_ON - repmat(initial_clock, size(RIGHT_ON)))./1000000; 
        R_light_ends = (RIGHT_OFF - repmat(initial_clock, size(RIGHT_OFF)))./1000000;
        R_light_starts_ends = [R_light_starts' R_light_ends'];       
        for R_light = 1:size(R_light_starts_ends,1)

            %find range
            [~,start_row] = min(abs(eptrials(:,1) - repmat(R_light_starts_ends(R_light, 1), size(eptrials(:,1)))));
            [~,end_row] = min(abs(eptrials(:,1) - repmat(R_light_starts_ends(R_light, 2), size(eptrials(:,1)))));

            %set rows in range
            eptrials(start_row:end_row, 13) = 2;
        end
    end
...
%Correct turn flags
eptrials = correct_turn_flags(eptrials);
    function eptrials = correct_turn_flags(eptrials)
    %uses position information to determine the rat's choice on the current
    %trial in order to fill in for missing flag information
    %
    
    %counter
    count = 0;
    
        for turn_trial = 1:max(eptrials(:,6))
            
            %turn_trial

            %RIGHT: if there are no visits to the left arm / rwd, but there are visits to the right arm / rwd
            if ~isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [5 7]),1)) && isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [4 6]),1))
               %set as right if not right already
               if not(mode(eptrials(eptrials(:,6)==turn_trial, 8)) == 2) 
                    eptrials(eptrials(:,6)==turn_trial, 8) = 2;
                    count = count + 1;
               end

            %LEFT: if there are visits to the left arm / rwd, but no visits to the right arm / rwd
            elseif isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [5 7]),1)) && ~isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [4 6]),1))

                %turn_trial
                %set as left if not left already
                if not(mode(eptrials(eptrials(:,6)==turn_trial, 8)) == 1) 
                    eptrials(eptrials(:,6)==turn_trial, 8) = 1;
                    count = count + 1;
                end

            %if there are visits to both
            elseif ~isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [5 7]),1)) && ~isempty(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), [4 6]),1))

                %turn_trial
                %min(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), 6:7), 1))
                
                %if they visit the left reward section first
                if eptrials(eptrials(:,1)==min(eptrials(eptrials(:,6)==turn_trial & ismember(eptrials(:,10), 6:7), 1)), 8) == 1
        ...
        if ~count==0
            %print warning
            display(strcat([num2str(count), ' bad or missing turn flag(s) calculated from position data']))
        end
    end
